...
-Test your building and contents limit logic on ONTAR05 as well, unique rate sheet
-The payment / renewal date replacement depends on the email template date being "Month , 2023".
It must be updated each year and the spacing must be the same for the replacement logic to work.
-The way you type in the renewal policy term must be "Month #, 2023 - Month #, 2024".
...
-The rate sheet logic for checking if a client has building and contents coverage is 95% of the time valid.
Sometimes if the ctrl C is too slow, it may incorrectly remove one of the coverage sections from the email, and you will just have to fix it.
-Before unpausing on the rate sheet for storing building and contents, make sure the cursor is at the top
so that it selects the first instance of those cells. It will technically work if the cursor is at the bottom, however
certain types of rate sheets have multiple instances of "Building" and "Contents" cells so it may not pick the one you want.
-If this script is being used by anyone else or the location of your "Renewal Policies" folder changes,
...
-When creating the renewal letter, the year on the attachment name is changed to 2023. In later years 
this will need to be updated to the respective year number (unless the template is changed to the year anyway).
*/
...
Send, {Down}{Left 2}%client_code% - %policy_number%{Down}^v
Pause
...
if (A_S = "y" or A_S = "y " or A_S = " y" or A_S = "Y" or A_S = "Y " or A_S = " Y"){
	...
	InputBox, premium, Account Balance, Enter the Red Account Balance (unless they have any other renewal owing as well)
	Send, !aa
	Sleep 500
	...
	InputBox, premium, Total Premium Owing, Enter the Red Premium owing (unless they have any other renewal owing as well)
	Send, !aa
	Pause
}
...
*/
SendMode Event ;FoxitPDF Reader is super slow, so an input mode change is required for it to keep up
...
Send, ^h%month% , 2023{Tab}%renewal_date%!a{Enter}{Escape}
Sleep 300
WinActivate ^%client_code%
Send, {Tab 5}{Enter}
Sleep 500
...
if (cert_copy = "n" or cert_copy = "n " or cert_copy = " n" or cert_copy = "N" or cert_copy = "N " or cert_copy = " N"){
	Send, ^fA certified copy!f{Escape}+{Down}{Backspace}
}
Sleep 200
